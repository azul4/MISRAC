...
int genCodeV(OpCode op, int v)
{
    ...
    printf("code[%d].opCode    = %d\n", cIndex, code[cIndex].opCode);
    printf("code[%d].u.value   = %d\n", cIndex, code[cIndex].u.value);
    return cIndex;
}
...
int genCodeR()
{
    ...
    code[cIndex].u.addr.level = GetBlockLevel();
    code[cIndex].u.addr.addr  = GetParamNum();
    return cIndex;
}

//====================================================================================
// 함수명 : checkMax
// 유일하게 cIndex를 증가시키는 곳. 
//
//====================================================================================
static void checkMax()
{
    printf("\nfunction = %s:: %s\n", __FILE__, __FUNCTION__);
    if (++cIndex < MAXCODE) {printf("cIndex(%d) < MAXCODE(%d)\n", cIndex, MAXCODE); return;}
